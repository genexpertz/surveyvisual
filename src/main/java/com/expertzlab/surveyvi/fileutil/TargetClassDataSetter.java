 ...
 */
class TargetClassDataSetter extends DataSetter{

    Class clazz;
    String[] hArray;
    String[] rArray;

    TargetClassDataSetter(Class clazz, String[] hArray, String[] rArray) {
      this.clazz =clazz;
      this.hArray = hArray;
      this.rArray = rArray;
    }
    public TargetClass run() {
        TargetClass tar =null;
        Class<?> loadedClass = null;
        try {
        loadedClass = Class.forName(clazz.getName());
        tar = (TargetClass) loadedClass.newInstance();
        for(int i = 0; i< hArray.length; i++) {

            Method m = clazz.getMethod("set" +capitalizeFirstLetter( hArray[i]), String.class);
            m.invoke(tar, rArray[i]);
        }
